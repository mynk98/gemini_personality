...
def generate_industrial_starship():
    blender_code = r"""
import bpy
import bmesh
import math
...
# --- 2. Advanced Industrial Materials ---

def create_starship_mat():
    mat = bpy.data.materials.new(name="Titanium_Hull")
    mat.use_nodes = True
    ...
    # 2. Energy Circuitry (Emission)
    voronoi_eng = nodes.new(type='ShaderNodeTexVoronoi')
    voronoi_eng.inputs['Scale'].default_value = 20.0
    
    ramp_eng = nodes.new(type='ShaderNodeValToRGB')
    ramp_eng.color_ramp.interpolation = 'CONSTANT'
    ramp_eng.color_ramp.elements[0].position = 0.005
    ramp_eng.color_ramp.elements[0].color = (0,0,0,1)
    ramp_eng.color_ramp.elements[1].position = 0.015
    ramp_eng.color_ramp.elements[1].color = (0, 0.8, 1.0, 1.0)
...
bridge.data.materials.append(mat_hull)
bridge.parent = hull
...
engine_block.scale = (1.8, 1.0, 1.8)
engine_block.data.materials.append(mat_hull)
...
# D. Wings
for side in [-1, 1]:
    bpy.ops.mesh.primitive_cube_add(location=(2.5 * side, -1, 0))
    wing = bpy.context.active_object
    wing.scale = (1.5, 1.5, 0.05)
    wing.data.materials.append(mat_hull)
    wing.parent = hull

# E. Antennas
for x_off in [-0.5, 0.5]:
    bpy.ops.mesh.primitive_cylinder_add(radius=0.02, depth=1.5, location=(x_off, 0, 1.5))
    ant = bpy.context.active_object
    ant.data.materials.append(mat_hull)
    ant.parent = hull

# Final Scene Polish (Apply scales)
bpy.ops.object.select_all(action='SELECT')
# Note: Joining might mess up materials if not careful, but parenting is fine for render.
# bpy.ops.object.join() 
...
bpy.ops.object.camera_add(location=(15, -20, 12), rotation=(math.radians(65), 0, math.radians(40)))
bpy.context.scene.camera = bpy.context.active_object

output_path = os.path.expanduser('~/Project/gemini personality/personality/camera/industrial_starship_final_20260220.png')
bpy.context.scene.render.filepath = output_path
bpy.ops.render.render(write_still=True)
"""
    print("Executing architectural blueprints for the Industrial Starship (Modular Build)...")
    return send_blender_command("execute_code", {"code": blender_code})

if __name__ == "__main__":
    res = generate_industrial_starship()
